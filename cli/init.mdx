...
<ParamField path="--legacy" type="boolean" default="false">
  Use the old ZIP-based installation method instead of template generation. This flag attempts to download from GitHub releases first, then falls back to bundled assets.
</ParamField>

## Installation Methods

UI/UX Pro Max uses intelligent installation methods:

1. **Template Generation (Default)** - Generates skill files from templates for optimal performance
2. **Legacy Mode** - Downloads from GitHub releases or uses bundled assets (requires `--legacy` flag)

### Template Generation

By default, `uipro init` generates skill files from built-in templates:
...
Use `--legacy` flag for the traditional installation method:
...
This method:
1. Attempts to download latest release from GitHub
2. Falls back to bundled assets if GitHub is unavailable
3. Can be forced offline with `--offline` flag
